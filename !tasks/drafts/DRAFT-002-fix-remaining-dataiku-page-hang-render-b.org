#+TITLE: Fix remaining dataiku page hang - render batching and init cascade
#+STATE: DONE
#+SESSION_ID: 1490dfde508bdb19
#+TRANSCRIPT_PATH: [[file:/home/user/.spell/agent/sessions/-code-ora-verse/2026-03-13T08-22-25-858Z_1490dfde508bdb19.jsonl]]
#+CUSTOM_ID: DRAFT-002-fix-remaining-dataiku-page-hang-render-b
#+EFFORT: 4h
#+PRIORITY: #A
#+LAYER: frontend
...
The dataiku homepage still hangs on load despite the RC1-RC4 fixes in commit f8362826.
The previous fixes addressed MutationObserver batching, scroll coalescing, bind dirty-checks,
and state machine CSS emission. But the page still locks up.

* Root Causes (confirmed via source analysis)

** RC6: each-with-templates render() inserts to live DOM per-item [CRITICAL]

~stdlib/primitives/template.st~ lines 746-784: ~render()~ does:
1. ~containerEl.innerHTML = ''~ — synchronously removes all children, triggering MutationObserver cleanup for every descendant (ST.dispose per element)
2. ~data.forEach(... containerEl.appendChild(el))~ — each appendChild goes directly to live DOM

With 3 customer @each containers resolving simultaneously (same data source "customers"),
all 3 call render() synchronously in sequence from ST.setData's listener loop.
Each render = 4 items × template factory + 4 appendChild calls to live DOM.
That's 12 appendChild calls triggering 12 MutationObserver callbacks.

** RC7: _flushInit still O(selectors × nodes × descendants) [HIGH]

~public/runtime/st.js~ lines 124-148: ~_flushInit()~ iterates ALL ~_selectorInitializers~
(~86 for dataiku) and for EACH pending node calls ~el.querySelectorAll(selector)~.
When a pending node is a large template section (hero = 50+ elements, product tabs = 100+),
querySelectorAll traverses the entire subtree 86 times.

Furthermore, ~_flushInit~ sets ~_initScheduled = false~ at the TOP (line 125) BEFORE processing.
If any init callback triggers DOM mutations (appendChild), MutationObserver fires, ~_scheduleInit~
sees ~_initScheduled === false~, and queues ANOTHER ~_flushInit~ microtask. This creates cascading
flush cycles: flush 1 processes initial nodes → inits add more nodes → flush 2 processes those → etc.

** RC8: state-transition double document listener (unfixed RC5) [MEDIUM]

~stdlib/primitives/state.st~ lines 182-186: each ~@transition~ registers its handler on BOTH
~%&el~ and ~document~. With 28 transitions across 4 state machines, that's 28 document-level
event listeners. Every ~@emit~ event dispatched to document reaches all 28 handlers.

** RC9: Dead fetch for resources data [LOW]

~site/dataiku/index.st~ declares ~@data resources~ but the ~&dku-resources~ template uses
hardcoded HTML with no ~@each~. The fetch fires and its result is discarded. Wasted network
request + wasted event dispatch. Low priority.

* Approach

** ITEM Phase 1: DocumentFragment batching in each-with-templates render()
:PROPERTIES:
:EFFORT: 45m
:FILES: stdlib/primitives/template.st
:END:

Replace the per-item ~containerEl.appendChild(el)~ with DocumentFragment batching:

#+begin_src javascript
const render = (data) => {
  if (!Array.isArray(data)) return;
  
  // Build all items into a fragment (off-DOM)
  ...
  data.forEach((itemData, index) => {
    ...
    for (const invocation of templateInvocations) {
      ...
      if (el) {
        ...
        fragment.appendChild(el);  // off-DOM, no MutationObserver
  ...
  // Single DOM operation: clear + insert
...
Key: ~replaceChildren(fragment)~ does ONE DOM mutation instead of N+1
(1 innerHTML clear + N appendChild). The MutationObserver fires once with
all added nodes in a single mutation record.

*** Verification
- ~cargo run --features headless -- test tests/ --headless~ — all headless tests pass
- @each template injection still works
- Elements get correct data-key and data-index attributes

** ITEM Phase 2: Optimize _flushInit — single-pass document query
:PROPERTIES:
:EFFORT: 1h
:FILES: public/runtime/st.js
:END:

Two changes:

*** 2a: Move _initScheduled = false to END of flush

Currently ~_initScheduled = false~ is set at line 125 (top). This allows
cascading microtask scheduling during flush. Move it to the end:
...
ST._flushInit = function() {
  ...
  // If new nodes were added during processing, schedule another flush
...
This prevents cascading flushes from interleaving. Nodes added during
processing accumulate in the new ~_pendingInit~ Set and get a single
follow-up flush.

*** 2b: Query document once per selector instead of per-node

The current approach iterates ~(selectors × pending_nodes)~ and calls
~el.querySelectorAll(selector)~ per node. For large subtrees this is
expensive. Instead, since every init function has a guard
(~el._st_init_{hash}~), we can query document.body once per selector
and rely on the guard to skip already-initialized elements:
...
ST._flushInit = function() {
  ...
  for (const { selector, init } of ST._selectorInitializers) {
    // Check if any pending node or its descendants might match
    // Use document-level query — init guard prevents double-init
    document.querySelectorAll(selector).forEach(child => {
      try { init(child); } catch(e) {
        if (ST.debug) console.error('[ST] Init error for', selector, e);
      }
    });
  }
...
Wait — this is WORSE. It queries ALL matching elements in the document, not just
the newly added ones. The init guard prevents re-initialization but the query itself
is expensive.

Better approach: use the pending nodes as roots for targeted queries:
...
ST._flushInit = function() {
  ...
  // Deduplicate: remove nodes that are descendants of other pending nodes
  // (their parent will cover them via querySelectorAll)
  const roots = [];
...
The deduplication step is O(n²) on pending count but pending is typically small (< 20).
It eliminates redundant querySelectorAll calls on nodes that are children of other pending
nodes.

*** Verification
- ~cargo test --lib~ — unit tests pass
- ~cargo run --features headless -- test tests/ --headless~ — headless tests pass
- MutationObserver still initializes dynamically added elements
- No double-initialization (init guards still present in registerSelectorInit)

** ITEM Phase 3: Coalesce state-transition document listeners
:PROPERTIES:
:EFFORT: 1h
:FILES: stdlib/primitives/state.st
:END:

Replace per-transition ~document.addEventListener~ with a shared registry,
similar to the RC2 scroll coalescing fix:

#+begin_src javascript
// Shared transition registry (initialized once)
if (!window.__stTransitionHandlers) {
  window.__stTransitionHandlers = {};
}
const eventName = %on;
if (!window.__stTransitionHandlers[eventName]) {
  window.__stTransitionHandlers[eventName] = [];
  document.addEventListener(eventName, (e) => {
    const handlers = window.__stTransitionHandlers[eventName];
    for (let i = handlers.length - 1; i >= 0; i--) {
      if (!handlers[i](e)) {
        handlers.splice(i, 1); // remove if handler returns false (element removed)
      }
    }
  });
}
const transitionEntry = (e) => {
  if (!document.contains(%&el)) return false; // element removed from DOM
  handler(e);
  return true;
};
window.__stTransitionHandlers[eventName].push(transitionEntry);
#+end_src

This reduces 28 document-level listeners to ~10 (one per unique event name:
hero-tab-ai-success, hero-tab-customers, hero-tab-ecosystem, tab-unite, tab-agents,
tab-govern, tab-analytics, tab-scale, showcase-jnj, showcase-roche, showcase-novartis,
showcase-aviva, showcase-next, showcase-prev, testimonials-next, testimonials-prev).

Still keep element-level listener for direct element events (clicks etc.).
Only coalesce the document-level cross-element dispatch.

*** Verification
- ~cargo run --features headless -- test tests/ --headless~ — headless tests pass
- State machines still transition correctly (product tabs, showcase, testimonials)
- @emit events still reach cross-element state machines

** ITEM Phase 4: Remove dead resources fetch
:PROPERTIES:
:EFFORT: 15m
:FILES: site/dataiku/index.html
:END:

Remove the ~../../spacetime.css~ and ~../../spacetime.js~ references from index.html
(they're 0-byte files; the dev server injects the real runtime). These are leftovers
from the original commit that were restored with git.

Also check whether the ~@data resources~ declaration is actually used by any @each.
If not, either remove it or leave it (it's a minor network request).

*** Verification
- Dev server page loads the same
...
| File | Change |
|------+--------|
| ~stdlib/primitives/template.st~ | DocumentFragment batching in render() (RC6) |
| ~public/runtime/st.js~ | Fix _flushInit cascade + dedup pending nodes (RC7) |
| ~stdlib/primitives/state.st~ | Coalesce document-level transition listeners (RC8) |
| ~site/dataiku/index.html~ | Remove 0-byte spacetime.js/css refs (RC9) |

* Verification (End-to-End)

1. ~cargo test --lib~ — unit tests pass
2. ~cargo test --test integration_tests~ — integration tests pass (same 1 pre-existing failure)
3. ~cargo run --features headless -- test tests/ --headless~ — headless tests pass
4. ~cargo run -- check site/dataiku/~ — all 19 files pass
5. ~cargo run -- serve site/dataiku/~ — page loads without hang in Firefox
6. ~cargo run -- serve site/adovasio/~ — no regression
7. State machines work: product tabs click-to-switch, showcase carousel arrows, testimonials auto-rotate

* Risks

- Phase 1 (~replaceChildren~) changes render() behavior — existing @each tests must still pass. ~replaceChildren~ is supported in all modern browsers.
- Phase 2 (_flushInit dedup) changes init ordering — init functions should be idempotent due to guards, but verify.
- Phase 3 (transition coalescing) changes event dispatch semantics — handlers still fire for the right events, just from a shared listener instead of individual ones.
