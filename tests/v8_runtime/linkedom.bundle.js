(() => {
  ...
  var require_entry = __commonJS({
    "entry.js"() {
      init_esm10();
      globalThis.linkedom = { parseHTML, DOMParser };
      var { document, window: window2 } = parseHTML("<html><body></body></html>");
      ...
      globalThis.CustomEvent = window2.CustomEvent;
      globalThis.MouseEvent = window2.MouseEvent || window2.Event;
      globalThis.KeyboardEvent = window2.KeyboardEvent || window2.Event;
      globalThis.DragEvent = window2.DragEvent || window2.Event;
      // Hoist MutationObserver to the global scope. LinkeDOM exposes it as
      // `window.MutationObserver` (a per-document getter), but st.js installs its
      // dynamic-node auto-init observer behind `typeof MutationObserver !== 'undefined'`
      // at the GLOBAL scope. Without this hoist that guard is false, the observer is
      // never connected, and instances appended via Spacetime.invokeTemplate / @each
      // never auto-mount their body directives (class toggle / content binding / @on)
      // — the headless-only failure that masked an otherwise-correct production path.
      globalThis.MutationObserver = window2.MutationObserver;
      ...
      console.log("LinkeDOM initialized");
    }
  });
  require_entry();
})();
